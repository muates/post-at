package com.muates.postservice.producer.impl;
...
public class SocialInteractionNotificationProducer implements KafkaProducer<Long, SocialInteractionNotificationAvro> {
    ...
    @Override
    public void send(String topicName, Long key, SocialInteractionNotificationAvro value) {
        LOGGER.info("Sending message= '{}' to topic= '{}'", value, topicName);
        ListenableFuture<SendResult<Long, SocialInteractionNotificationAvro>> result = kafkaTemplate.send(topicName, key, value);
        addCallback(topicName, value, result);
    }
